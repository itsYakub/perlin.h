...
static inline double __perlin_floor(double a) { return ((double) ((int) a)); }


static inline double __perlin_fade(double t) { return (t * t * t * (t * (t * 6.0 - 15.0) + 10.0)); }


static inline double __perlin_lerp(double a, double b, double t) { return ((b - a) * t + a); }
...
PERLINAPI double perlin2D(double x, double y) {
    /* TODO:
     *  Tweak the values so that it outputs the proper perlin noise values [-1.0 - 1.0]
     * */
    return (perlin3D(x, 0.0, y));
}


PERLINAPI double perlin3D(double x, double y, double z) {
    int X = (int) __perlin_floor(x) & 255,
        Y = (int) __perlin_floor(y) & 255,
        Z = (int) __perlin_floor(z) & 255;
    ...
    int A = g_perm[X    ] + Y, AA = g_perm[A] + Z, AB = g_perm[A + 1] + Z,
